function obj = new_paint(obj,incl_con,rr,p,panels2,nii_name, params)
...
%Removes warning of constant ZData of contour (lines 315 and 330)
...
if params.refreshf
    ...
    if p == 1 && rr
        ...
        save(sprintf('yxratio_%s.mat', nii_name),'yxratio');
    elseif p == 1 && ~rr
       yxratio = length(ymm)*dims(Y,2)/(length(xmm)*dims(X,2));
    else
        load(sprintf('yxratio_%s.mat', nii_name));
    end
...
%itypes = {obj.img(:).type};
itypes = obj.img(:);            %JM edit
itypes = {itypes.type};         %JM edit
    ...
    for j=1:length(contimgs)
        ...
        if any(any(isfinite(i1)))
            ...
            if mars_struct('isthere', thisimg, 'contours')
                if incl_con
                    [~,h] = contour(axisd(i), i1, thisimg.contours, linespec);
                end
            else
                if ~rr
                    if incl_con
                        [~,h] = contour(axisd(i),i1,'cyan');
                    else
                        i3 = zeros(size(i1,1),size(i1,2));
                        [~,h] = contour(axisd(i),i3,'black');
                    end
                else
                    if incl_con
                        load(sprintf('i2s_%s.mat', nii_name))
                        [~,~] = contour(axisd(i),i1,'red');
                        hold on
                        [~,h] = contour(axisd(i),i2s{i},'cyan');
...
if ~rr
    i2s = i1s;
    save(sprintf('i2s_%s.mat', nii_name),'i2s');
end
% keyboard
